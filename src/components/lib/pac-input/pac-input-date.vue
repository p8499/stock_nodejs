<template>
    <div class="pac-input">
        ...
        <pac-datetime-picker precision="date" :value="value === null? new Date() : value" @input="pick($event)"
...
<script lang="ts">
    ...
    @Component({name: 'pac-input-date'})
    export default class PacInputDate extends PacInputCommon {
        ...
        format(value: Date | null): string {
            return value !== null ? this.dateToString(value) : '';
        }

        pickerShown = false;

        pick(date: Date) {
            this.rawValue = this.dateToString(date);
        }

        //check的相关属性
        @Watch('rawValue')
        onRawValueChange(n: any, o: any): void {
            this.check();
        }
        ...
        @Watch('min')
        onMinChange(n: any, o: any): void {
            this.check();
        }

        @Watch('max')
        onMaxChange(n: any, o: any): void {
            this.check();
        }
        ...
        async check(): Promise<void> {
            ...
            //检测日期格式不正确
            if (this.rawValue !== '' && !this.recognizeDate(this.rawValue)) {
                this.emit(null, `${this.label !== null ? this.label : ''}日期格式不正确)`);
                return
            }
            //日期规整化
            const normalized = this.rawValue !== '' ? this.normalizeDate(this.rawValue) : '';
            if (normalized !== this.rawValue) {
                this.rawValue = normalized;
                return;
            }
            //检测日期意义不正确
            if (this.rawValue !== '' && !this.validateDate(this.rawValue)) {
                this.emit(null, `${this.label !== null ? this.label : ''}日期格式不正确`);
                return
            }
            //检测超出值范围
            const date = this.rawValue !== '' ? this.stringToDate(this.rawValue) : null;
